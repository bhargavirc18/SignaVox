...
# # Function to run your Python application when the button is clicked
# @app.route('/run_application', methods=['POST'])
# def run_application():
#     # Put the command to run your Python application here
#     # For example:
#     subprocess.Popen(["python", "app.py"])
#     # Replace 'your_python_script.py' with the actual filename of your Python application
#     return "Python application started!"
...
# json_file = open("absolute/path/to/model.json", "r")
...
def hover_handler(event, x, y, flags, param):
    ...
    if 310 <= x <= 520 and 455 <= y <= 485:
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            cv2.putText(frame, "<- Backspace", (598, 479), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
            back_space = True
        else:
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        #             combineWord = combineWord[:-1]  # Remove the last character
        ...
        #     sentence = sentence[:-len(word[-1])] if len(word) > 0 else sentence
        ...
        cv2.putText(enlarge_frame, "Output: - " + letter_detected, (3, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(enlarge_frame, "Word: - " + combineWord, (3, 475), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(enlarge_frame, "Sentence: - " + sentence, (3, 515), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)

        cv2.rectangle(enlarge_frame, (310, 455), (520, 485), (245, 117, 16), -1)
        cv2.putText(enlarge_frame, "<- Backspace", (320, 479), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
